...
namespace XCYN.Common
{
    public class Utils
    {
        ...
        {
            string word = @"*|and|exec|insert|select|delete|update|drop|create|where|count|master|truncate|declare|char(|mid(|chr(|'";
            if (InText == null)
                return false;
        ...
        /// <param name="array">查找的数组(必须是正序排序)</param>
        /// <returns></returns>
        ...
        /// <param name="content">内容</param>
        /// <returns></returns>
        public static string SmsSend(string username, string password, string mobiles, string content)
        {
            username = System.Web.HttpUtility.UrlEncode(username, Encoding.GetEncoding("utf-8"));
            content = System.Web.HttpUtility.UrlEncode(content + "【武汉博大医院】", Encoding.GetEncoding("utf-8"));
            //content = System.Web.HttpUtility.UrlEncode(content + "【造就科技】", Encoding.GetEncoding("utf-8"));
            string taskid = DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss").Replace(":", "").Replace("-", "").Replace(" ", "");
            taskid = taskid + "_000001";
            string sendurl = string.Format("{0}?op=sendsms&username={1}&password={2}&smsmes={3}&smsphone={4}&taskid={5}"
                , httpstr
                , username
                , password
                , content
        ...
        /// <returns></returns>
        public static string GetImageSize(HttpServerUtility Server, string path)
        {
            try
            {
                ...
                if (reg.IsMatch(path))
                {
                    ...
                    fs = myresponse.GetResponseStream();
                }
                else
                {
                    //如果是本地路径
                    path = Server.MapPath(path);
                    fs = File.OpenRead(path); //OpenRead
                }
