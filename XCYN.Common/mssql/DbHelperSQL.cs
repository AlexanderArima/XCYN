...
namespace XCYN.Common
{
    public abstract class DbHelperSQL
    {
        ...
        /// <returns>SqlDataReader</returns>
        public static object RunProcedure(string storedProcName, IDataParameter[] parameters)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
        ...
        public static DataTable RunProcedure_dt(string storedProcName, IDataParameter[] parameters)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
        ...
        public static bool Fresh_superiors_rank(string sql,string field_name)
        {
            ...
            try
            {
                ...
                ////1:add
                //for (int i = 1; i <= 10; i++)
                //    dt.Rows.Add(new object[] { i, i, "kk" + i });

                //SqlCommandBuilder ocb = new SqlCommandBuilder(sda);//自动生成相应的命令，这句很重要  
                //sda.UpdateCommand = ocb.GetInsertCommand();
                //sda.Update(dt);
                //dt.AcceptChanges();
                ...
                {
                    dt.Rows[i][field_name] = int.Parse(dt.Rows[i][field_name].ToString()) + 1;
                }
                dt.Rows[dt.Rows.Count - 1][field_name] = 1;
                //3:DELETE
                //for (int j = 0; j < dt.Rows.Count; j++)
                //{
                //    if (dt.Rows[j]["hitinfo"].ToString().Equals("test"))
                //    {
                //        dt.Rows[j].Delete();
                //        dt.Rows[j].Delete();
                //    }
                ...
                sda.UpdateCommand = ocb.GetUpdateCommand();
                sda.Update(dt);
                dt.AcceptChanges();
            }
            catch
