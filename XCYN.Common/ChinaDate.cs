...
namespace XCYN.Common
{
    public class ChinaDate
    {
        ...
        public static string GetYear(DateTime dt)
        {
            ...
            string str = string.Format("{0}", year);
            return str;
        }
        ...
        public static string GetSolarTermNextDay(DateTime dt)
        {
            DateTime dtBase = new DateTime(1900, 1, 6, 2, 5, 0);
            DateTime dtNew;
            double num;
            ...
            for (int i = 1; i <= 24; i++)
            {
                ...
                if (dtNew.DayOfYear > dt.DayOfYear)
                {
                    strReturn = string.Format("{0}[{1}]", JQ[i - 1], dtNew.ToString("yyyy-MM-dd"));
                    //strReturn = JQ[i - 1];
                    break;
                }
            }
        ...
        public static bool IsBeforeLiChun(DateTime date)
        {
            ...
            TimeSpan start_time = new TimeSpan(date.Ticks);
            TimeSpan end_time = new TimeSpan(lc_date.Ticks);
            if (end_time.Subtract(start_time).Days > 0)
            {
                return true;
            }
            else
        ...
        /// <returns></returns>
        private static DateTime GetLiChunDate(int year)
        {
            for (int i = 1; i <= 12; i++)
            {
                DateTime dtNow = new DateTime(year, i, 1);
                int days = DateTime.DaysInMonth(dtNow.Year, dtNow.Month);
                for (int j = 1; j <= days; j++)
                {
                    string nl_date = ChinaDate.GetChinaDate(new DateTime(year, i, j));
                    if (nl_date.Contains("立春"))
                        return Convert.ToDateTime(year + "-" + i + "-" + j);
                }
                //公历转农历
